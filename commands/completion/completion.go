...
func NewCmdCompletion(io *iostreams.IOStreams) *cobra.Command {
	...
	var completionCmd = &cobra.Command{
		...
		Long: `Generate shell completion scripts for dns-cli commands.
...
For example, for bash you could add this to your '~/.bash_profile':
	...
	eval "$(dns-cli completion -s bash)"

Generate a %[1]s_dns-cli%[1]s completion script and put it somewhere in your %[1]s$fpath%[1]s:
				dns-cli completion -s zsh > /usr/local/share/zsh/site-functions/_dns-cli
